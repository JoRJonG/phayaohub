...
import { Plus, Edit2, Trash2, Hotel, Eye, Star, ImageIcon } from 'lucide-react';
...
const AdminAccommodations: React.FC = () => {
    ...
    const fetchGuides = async () => {
        ...
        try {
            ...
            const response = await fetch('/api/admin/guides?category=พัก', {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const response = await fetch(url, {
                ...
                body: JSON.stringify({ ...formData, image_url: uploadedImageUrl, category: 'พัก' })
            });
    ...
    return (
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                <div>
                    <h1 className="text-2xl font-bold text-slate-800">จัดการที่พัก</h1>
                    <p className="text-slate-500">จัดการข้อมูลที่พัก โรงแรม รีสอร์ท ในพะเยา</p>
                </div>
                <button
                    ...
                    <table className="w-full text-left border-collapse">
                        <thead>
                            <tr className="bg-slate-50 border-b border-slate-200 text-slate-600 text-sm uppercase tracking-wider">
                                <th className="p-4 font-semibold">ที่พัก</th>
                    ...
                    <div className="bg-white rounded-2xl w-full max-w-2xl shadow-2xl max-h-[90vh] flex flex-col">
                        <div className="p-6 border-b border-slate-100 flex justify-between items-center">
                            <h2 className="text-xl font-bold text-slate-800">
                                {isEditing ? 'แก้ไขที่พัก' : 'เพิ่มที่พัก'}
                            </h2>
                            ...
                            <form onSubmit={handleSubmit} className="space-y-4">
                                ...
                                <div className="flex items-center gap-4">
                                    <label className="flex items-center gap-2 cursor-pointer">
                                        ...
                                        />
                                        <span className="text-sm text-slate-700">แสดงเป็นที่พักแนะนำ</span>
                                    </label>
                                </div>
                                <div>
